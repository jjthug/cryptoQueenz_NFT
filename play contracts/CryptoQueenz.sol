...
struct DutchAuctionConfig {
  ...
  uint256 public constant TOTAL_SUPPLY = MAX_OWNER_RESERVE + CRYPTO_BATZ_SUPPLY;
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, (((tokenId + randomizedStartIndex) % TOTAL_SUPPLY) + 1).toString())) : "";
    }

  function setBaseURI(string calldata newBaseUri) external onlyOwner {
    abi.encode("Dummy URL");
